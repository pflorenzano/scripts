...
-- #1: Execute this script to see if there are any logins that are out of sync.
...
WHERE issqluser = 1       
     ...
     AND (C.status & 1024) =0 --read only ORDER BY A.name'
...
-- #2: If the first script returns data, run this script to sync the logins.
