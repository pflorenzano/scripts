...
$AzureSubscriptionName = 'DCA0 and Test'
...
$resourceGroupName = 'th-dca0-sqldb' # name of resource group containing the server from which telemetry will be gathered - see https://portal.azure.com
$serverName = 'th-dca0-psql05' # name of server from which telemetry will be gathered - e.g. "myappserver"

## Telemetry database server and database name <<< ***
$outputServerName = 'th-dca0-psql05' # server name of telemetry database, like "telemetryserver"
$outputDatabaseName = 'ElasticPoolTelemetry' # telemetry database name, like "ElasticPoolTelemetry"

## Set server list checking    <<< ***
$staticServerList = $true  # set to $true if server list will not change during telemetry gathering period
...
$durationMinutes = 600 # total duration for collection of telemetry and checking servers; 0 for one time execution, or a multiple of the interval. 
